...
public class GenericParticle extends SpriteBillboardParticle {
    ...
    public GenericParticle(ClientWorld world, WorldParticleEffect data, FabricSpriteProviderImpl spriteProvider, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
        ...
        Color.RGBtoHSB(
                (int) (255 * Math.min(1.0f, colorData.r1)),
                (int) (255 * Math.min(1.0f, colorData.g1)),
                (int) (255 * Math.min(1.0f, colorData.b1)),
                hsv1
        );
        Color.RGBtoHSB(
                (int) (255 * Math.min(1.0f, colorData.r2)),
                (int) (255 * Math.min(1.0f, colorData.g2)),
                (int) (255 * Math.min(1.0f, colorData.b2)),
                hsv2
        );
        ...
        if (spriteSet != null) {
            ...
            if (getSpritePicker().equals(SimpleParticleEffect.ParticleSpritePicker.FIRST_INDEX) || getSpritePicker().equals(
                    SimpleParticleEffect.ParticleSpritePicker.WITH_AGE)) {
                setSprite(0);
            }
    ...
    public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float tickDelta) {
        ...
        if (ClientConfig.DELAYED_RENDERING && textureSheet instanceof LodestoneWorldParticleTextureSheet sheet) {
