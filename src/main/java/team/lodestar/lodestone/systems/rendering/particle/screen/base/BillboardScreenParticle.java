...
public abstract class BillboardScreenParticle extends ScreenParticle {
    ...
    public void render(BufferBuilder bufferBuilder) {
        ...
        Vector3f[] vectors = new Vector3f[]{new Vector3f(-1.0F, -1.0F, 0.0F), new Vector3f(
                -1.0F,
                1.0F,
                0.0F
        ...
        bufferBuilder.vertex(vectors[0].x(), vectors[0].y(), quadZ)
                     .texture(u1, v1)
                     .color(this.rCol, this.gCol, this.bCol, this.alpha)
                     .next();
        bufferBuilder.vertex(vectors[1].x(), vectors[1].y(), quadZ)
                     .texture(u1, v0)
                     .color(this.rCol, this.gCol, this.bCol, this.alpha)
                     .next();
        bufferBuilder.vertex(vectors[2].x(), vectors[2].y(), quadZ)
                     .texture(u0, v0)
                     .color(this.rCol, this.gCol, this.bCol, this.alpha)
                     .next();
        bufferBuilder.vertex(vectors[3].x(), vectors[3].y(), quadZ)
                     .texture(u0, v1)
                     .color(this.rCol, this.gCol, this.bCol, this.alpha)
                     .next();
    }
