...
import team.lodestar.lodestone.mixin.accessor.RenderPhase$ShaderProgramAccessor;
...
public final class RenderHelper {
    ...
    public static ShaderProgram getShader(RenderLayer layer) {
        if (layer instanceof RenderLayer.MultiPhase multiPhase) {
            RenderLayer$MultiPhaseAccessor renderLayer$multiPhaseAccessor = (RenderLayer$MultiPhaseAccessor) (Object) multiPhase;
            RenderLayer$MultiPhaseParametersAccessor renderLayer$multiPhaseParametersAccessor = (RenderLayer$MultiPhaseParametersAccessor) (Object) renderLayer$multiPhaseAccessor.getPhases();
            RenderPhase$ShaderProgramAccessor shaderProgramAccessor = (RenderPhase$ShaderProgramAccessor) renderLayer$multiPhaseParametersAccessor.getProgram();
            Optional<Supplier<ShaderProgram>> shader = shaderProgramAccessor.getSupplier();
            if (shader.isPresent()) {
    ...
    public static RenderPhase.Transparency getTransparency(RenderLayer layer) {
        if (layer instanceof RenderLayer.MultiPhase multiPhase) {
            RenderLayer$MultiPhaseAccessor renderLayer$multiPhaseAccessor = (RenderLayer$MultiPhaseAccessor) (Object) multiPhase;
            RenderLayer$MultiPhaseParametersAccessor renderLayer$multiPhaseParametersAccessor = (RenderLayer$MultiPhaseParametersAccessor) (Object) renderLayer$multiPhaseAccessor.getPhases();
            return renderLayer$multiPhaseParametersAccessor.getTransparency();
        }
        return null;
    }
    ...
    public static void drawLineBetween(VertexConsumerProvider buffer, MatrixStack ms, Vec3d local, Vec3d target, float lineWidth, int r, int g, int b, int a) {
        ...
        float rotX = (float) MathHelper.atan2(
                target.y - local.y,
                MathHelper.sqrt((float) (distX * distX + distZ * distZ))
        );
