...
public class BlockHelper {
    ...
    public static BlockPos loadBlockPos(NbtCompound tag, String extra) {
        return tag.contains(extra + "_X") ? new BlockPos(
                tag.getInt(extra + "_X"),
                tag.getInt(extra + "_Y"),
                tag.getInt(extra + "_Z")
        ) : null;
    ...
    public static <T> Stream<T> getBlockEntitiesStream(Class<T> type, World level, BlockPos pos, int x1, int y1, int z1, int x2, int y2, int z2) {
        return getBlockEntitiesStream(
                ...
                level,
                new Box(
                        (double) pos.getX() + 1.5f + x1,
                        (double) pos.getY() + 1.5f + y1,
                        (double) pos.getZ() + 1.5f + z1,
                        (double) pos.getX() + 0.5f + x2,
                        (double) pos.getY() + 0.5f + y2,
                        (double) pos.getZ() + 0.5f + z2
                )
        );
    }
